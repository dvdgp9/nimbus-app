<x-guest-layout>
    ...
    <div class="mb-8 text-center">
        ...
        <p class="text-cyan-100 text-sm">Accede a tu cuenta de Nimbus</p>
    </div>
    ...
    <div class="relative mb-6">
        ...
        <div class="relative flex justify-center text-sm">
            <span class="px-3 glass-subtle text-white font-medium">O continúa con email</span>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-between mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-white/20 bg-white/10 text-cyan-400 shadow-sm focus:ring-cyan-400 focus:ring-offset-transparent" name="remember">
                <span class="ms-2 text-sm text-white">Recuérdame</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-cyan-300 hover:text-cyan-200 hover:underline transition" href="{{ route('password.request') }}">
