...
<div class="page-container max-w-4xl">
  {{-- Page Header --}}
  <div class="mb-8">
    <a href="{{ route('templates.index', ['channel' => $template->channel]) }}" class="text-cyan-400 hover:text-cyan-300 transition inline-flex items-center gap-1 mb-4">
    ...
    <div class="page-header">
      ...
      <p>Modifica tu plantilla de {{ $template->channel === 'email' ? 'email' : 'SMS' }}</p>
    </div>
  </div>

  {{-- Form --}}
  <form method="POST" action="{{ route('templates.update', $template) }}" class="space-y-6" id="template-form">
    @csrf
    @method('PUT')
          ...
          <div class="border-t border-white/10 pt-4">
            <label class="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p class="text-xs text-white/40">Se usará automáticamente para los recordatorios de {{ $template->channel === 'email' ? 'email' : 'SMS' }}</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div class="bg-white/5 rounded-xl border border-white/10 p-6 sticky top-4">
          ...
          @if($template->isEmail())
            <div class="mb-3 text-sm">
              ...
              <span class="text-white/80" id="preview-subject">{{ $template->parseSubject(['appointment_summary' => 'Sesión de terapia']) }}</span>
            </div>
          @endif
    ...
    <div class="flex gap-3 justify-end">
      <a href="{{ route('templates.index', ['channel' => $template->channel]) }}" class="btn bg-white/5 hover:bg-white/10 text-white">
        Cancelar
      </a>
